...
- Engram persistence is owned through `workflow_state`. Subagents do not write Engram. The Lead may run read-only `mem_search` / `mem_context` / `mem_get_observation` at the start of Discovery to seed context from prior work; it never writes, and `workflow_state` stays canonical. Write/update/delete `engram_mem_*` tools remain blocked for every workflow agent.
