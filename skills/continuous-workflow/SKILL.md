...
Implementation track: record a non-protected delivery branch/worktree, present and record the implementation brief, record the verification plan, transition to implementation, delegate the approved package, inspect the diff and final evidence, record verification once, then in this order: present the result for the user's manual/visual review (`manual_confirm`, early), apply corrections if the user requests them (implementation → verification, reusing contract/matrix/brief), run the independent reviewer (pre-CI, `review_record`), record the commit (`post_ci`) and the CI result (`ci_status`), run post-CI corrections if any (review → implementation → verification → review) and re-review, then `ready`, explicit user confirmation, and `complete`.

The user's manual/visual review comes early — right after implementation and verification, before the independent review and the commit — because that is the cheapest place to catch what the user cares about. `ready` is enforced to require the independent reviewer passed on the current tree with zero unresolved findings, the complete capability matrix, and the user's manual review confirmation. Green tests/CI alone never make a change ready. Corrections reuse the contract, capability matrix, brief, and delivery setup; only a material scope/behavior change reopens the contract.
